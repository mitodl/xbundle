...
class XBundle(object):
    ...
    def import_metadata_from_directory(self, dir):
        ...
        # load about files
        for afn in glob.glob(join(dir, 'about/*')):
            try:
    ...
    def import_course_from_directory(self, dir):
        ...
        x = etree.parse(join(dir, 'course.xml')).getroot()
    ...
    def import_xml_removing_descriptor(self, dir, xml):
        ...
        if xml.tag in ['html','problem'] and fn: # special for <html filename="..." display_name="..."/>
            ...
            if not os.path.exists(join(dir, xml.tag, fn)):
                if '-' in fn:
                    fn = '%s/%s' % (fn.split('-',1)[0], fn)
            try:
                dxml = etree.parse(join(dir, xml.tag, fn), **options).getroot()
            except Exception as err:
    ...
    def export_to_directory(self, exdir='./', xml_only=False, newfmt=True):
        ...
        self.dir = self.mkdir(join(exdir, self.course_id()))
        if not xml_only:
        ...
        self.write_xml_file(join(self.dir, 'course.xml'), coursex)
    ...
    def export_meta_to_directory(self):
        ...
        '''
        pdir = self.mkdir(join(self.dir, 'policies'))
    ...
    def export_xml_to_directory(self, elem, dowrite=False):
        ...
        def write_xml(x):
            ...
            edir = self.mkdir(join(self.dir, x.tag))
            self.write_xml_file(join(edir, un + '.xml'), x)
            return un
